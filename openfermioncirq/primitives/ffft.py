...
class _F0Gate(cirq.TwoQubitMatrixGate):
    ...
    :math:`a_0^\dagger \lvert 00 \rangle = \lvert 10 \rangle` and
    :math:`a_1^\dagger \lvert 00 \rangle = \vert 01 \rangle`.
...
def ffft(qubits: Sequence[cirq.Qid]) -> cirq.OP_TREE:
    ...
    :math:`\hat{a}_n^\dagger`, :math:`n \in 1, 2, \dots, N` according to:
    ...
    where :math:`\tilde{a}_k^\dagger` are transformed operators and :math:`N` is
    ...
    :math:`a_0^\dagger \lvert 0.. \rangle = \lvert 1.. \rangle`,
    :math:`a_1^\dagger \lvert 0.. \rangle = \vert 01.. \rangle`,
    :math:`a_2^\dagger \lvert 0.. \rangle = \vert 001.. \rangle`, :math:`\dots`.
    It works only for qubit sequences which are power of 2; arbitrary input
    lengths are not supported.

    The gate count of generated circuit is :math:`\theta(N^2)`, generated
    circuit depth is :math:`\theta(N)` and distinct gates count is
    :math:`\theta(\log(N))`.

    An equivalent circuit can be generated using the
    `openfermioncirq.bogoliubov_transform` function with appropriately prepared
    ...
    .. testcode::
        ...
        def ffft(qubits):
            ...
            return openfermioncirq.bogoliubov_transform(
                qubits, fourier_transform_matrix(len(qubits)))

    The advantage of circuit generated by the FFFT algorithm over the one
    created with the `bogoliubov_transform` is that significantly smaller
    variety of gates is created. It is equal to :math:`\theta(\log(N))` which is
    smaller than :math:`O(N^2)` generated by `bogoliubov_transform`.

    This implementation of FFFT is based on standard power of 2 Cooley-Tukey
    algorithm and adapted to nearest neighbourhood connectivity.
    ...
    # This follows standard power-of-2 Cooley-Tukey algorithm. The first step
    ...
    # The second part is to perform n/2 FFFTs of size 2 on the pairs of odd and
    ...
    # The result of performing FFFT on k-th pair is a new pair of operators with
