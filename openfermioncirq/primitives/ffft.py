...
def ffft(qubits: Sequence[cirq.Qid]) -> cirq.OP_TREE:
    ...
    :math:`\theta(N_1^2 + N_2^2 + \dots + N_n^2)`, where
    :math:`N = N_1 N_2 \dots N_n` is prime decomposition of :math:`N`. In a case
    where :math:`N` is some power of 2, it reduces to :math:`\theta(\log(N))`.
    ...
    is created, which is :math:`O(N^2)` in case of pure `bogoliubov_transform`.
