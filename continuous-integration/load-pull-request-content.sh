...
# This script-helper fetches a pull request from the
# quantumlib/openfermion-cirq repository and defines a set_status function for
# informing github of the outcome of local checking.
#
...
# with permission to update status values on the openfermin-cirq repo, then the
# status of the tests will be reported to github and shown within the PR. If
# not set, the tests merely run locally. If this argument is not set, the
...
function set_status () {
  ...
  if [ -n "${pull_id}" ] && [ -n "${access_token}" ]; then
    ...
    url="https://api.github.com/repos/quantumlib/openfermion-cirq/statuses/${commit_id}?access_token=${access_token}"
...
if [ -z "${pull_id}" ]; then
  ...
  origin="git@github.com:quantumlib/openfermion-cirq.git"
  cd "${work_dir}"
  git init --quiet
  git fetch git@github.com:quantumlib/openfermion-cirq.git pull/${pull_id}/head --depth=1 --quiet
